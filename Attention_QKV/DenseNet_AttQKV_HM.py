...
class AttentionNet(nn.Module):
    def __init__(self, channels, hidden_layers=1, kernel_att=3, stride_att=1):
        ...
        self.c1 = nn.Conv2d(channels, channels, kernel_size=self.kernel_att, stride=self.stride_att, padding=self.padding_att)
        self.relu = nn.ReLU()
        self.c2 = nn.Conv2d(channels, channels, kernel_size=self.kernel_att, stride=self.stride_att, padding=self.padding_att)
        self.c3 = nn.Conv2d(channels, channels, kernel_size=self.kernel_att, stride=self.stride_att, padding=self.padding_att)
        self.c4 = nn.Conv2d(channels, channels, kernel_size=self.kernel_att, stride=self.stride_att, padding=self.padding_att)
        self.c5 = nn.Conv2d(channels, channels, kernel_size=self.kernel_att, stride=self.stride_att, padding=self.padding_att)
        self.sig = nn.Sigmoid()
...
class DenseNet_att_QKV_HM(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #Attention
        self.conv_embedding = AttentionNet(1664, self.hidden_layers_att, self.kernel_att, self.stride_att)
        ...
        self.conv_reducer_att_output = nn.Conv2d(int((self.dv)), int(8*self.Nh), 1)
        ...
        self.classifier = nn.Linear(8 * self.Nh * 16 * 16, 1)
    ...
    def forward(self, x, no_grad=False):
        ...
        out_att = torch.matmul(flat_Z, V.transpose(2, 3))
        #out_att = torch.matmul(flat_Z, V)
        out_att = torch.reshape(out_att, (B_z, self.Nh, self.dv // self.Nh, 16, 16))
        #print('einsum', out_att)

        Bo, Headso, Fo, H, W = out_att.size()
        #print('out', out.shape)
        ...
        out_att = self.stack_heads_2d(out_att)
        if (no_grad is False) and (self.training is False):
            print('Gradients Hooked')
            h = out_att.register_hook(self.activations_hook)


        out_att = self.conv_reducer_att_output(out_att)
        out_att = F.relu(out_att, inplace=True)
        ...
        radiographical_findings = self.classifier(out_att)
