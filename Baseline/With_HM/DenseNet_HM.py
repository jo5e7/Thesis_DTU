...
class DenseNet_MH(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 32, 32),
        ...
        # First convolution
        self.features_conv = nn.Sequential(OrderedDict([
        ...
        for i, num_layers in enumerate(block_config):
            ...
            self.features_conv.add_module('denseblock%d' % (i + 1), block)
            num_features = num_features + num_layers * growth_rate
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
                self.features_conv.add_module('transition%d' % (i + 1), trans)
                num_features = num_features // 2

        # Final batch norm
        self.features_conv.add_module('norm5', nn.BatchNorm2d(num_features))
