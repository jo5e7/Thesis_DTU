...
class Trainable_Model_AttQKV:
    ...
    def get_model_accuracy(self, model, data_loader, data_name, score_type, save_samples=False):
        ...
        if save_samples:
            ...
            avg_precision_score = Prediction_scores.get_avg_precision_score(all_y, all_y_pred)
            self.log('avg_precision_score: ' + str(avg_precision_score))
            tn, fp, fn, tp = Prediction_scores.get_confusion_matrix(all_y, all_y_pred)
            self.log('tn: ' + str(tn))
            self.log('fp: ' + str(fp))
            self.log('fn: ' + str(fn))
            self.log('tp: ' + str(tp))
    ...
    def train_Att(self, epochs=100, tolerance=2):
        ...
        if self.test_loader is not None:
            #del net
            sd = torch.load(self.name + '/' + self.name + '.pth', map_location=lambda storage, loc: storage)
            net = loadSD_densenet_attQKV_hm_169(model_state_dict=sd, hidden_layers_att=self.hidden_layers_att,
                                                bp_elementwise=self.bp_att, dq=self.dq, dv=self.dv,
                                                Att_heads=self.Att_heads, kernel_att=self.kernel_att,
                                                stride_att=self.stride_att, non_linearity_att=self.non_linearity_att,
