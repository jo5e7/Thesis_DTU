...
if vars(args)['root'] is not None:
    ...
    dv = int(vars(args)['dv'])
    Nh = int(vars(args)['Nh'])
    selfAtt = bool(vars(args)['selfAtt'])
    non_linearity = vars(args)['non_linearity']
...
save_heatmaps(bp_pos=bp_pos, hidden_layers_att=hidden_layers_att, dq=dq, dv=dv,
                                                Att_heads=Nh, kernel_att=kernel_att,
                                                stride_att=stride_att, non_linearity_att=non_linearity, self_att=selfAtt,
                  model_url=model_url, save_path=save_parh)